...
int main(int argc, char *args[])
{
    ...
    Shader shader("4.2.texture.vs", "4.2.texture.fs");
    ...
    stbi_set_flip_vertically_on_load(true); // igual que en LearnOpenGL 4.2
    ...
    if (data) {
        ...
        SDL_Log("No se pudo cargar resources/textures/container.jpg");
    }
    ...
    data = stbi_load("resources/textures/awesomeface.png", &w, &h, &n, 0);
    if (data) {
        glTexImage2D(GL_TEXTURE_2D, 0, GL_RGBA, w, h, 0, GL_RGBA, GL_UNSIGNED_BYTE, data);
        glGenerateMipmap(GL_TEXTURE_2D);
    } else {
        SDL_Log("No se pudo cargar resources/textures/awesomeface.png");
    }
    stbi_image_free(data);

    // Asignar unidades a los samplers del shader (solo una vez)
    shader.use();
